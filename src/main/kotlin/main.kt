...
val debug by parser
    .option(ArgType.Boolean, shortName = "d", fullName = "debug", description = "show debug information")
    .default(false)
...
suspend fun readTwitter(cacheDir: File, client: HttpClient, screenName: String): JsonArray? {
    ...
    try {
        ...
        if (now - lastModified < 240000L) {
            if (debug) log.i("$screenName: readTwitter: read from cache.")
    ...
    if (debug) log.i("$screenName: readTwitter read from server.")
